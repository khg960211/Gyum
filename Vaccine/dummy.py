...
class KavMain:
    ...
    # 리턴값 : 0 - 성공, 0 이외의 값 - 실패
    #---------------------------------------------------------------------------
    def init(self, plugins_path): # 플러그인 엔진초기화
        # 진단/치료하는 악성코드 이름
    ...
    def scan(self, filehandle, filename):
        try:
            ...
            fp = open(filename)
            buf = fp.read(len(self.dummy_pattern)) # 패턴은 49 Byte zmrl
            fp.close()

            # 악성코드 패턴을 비교한다.
            if buf == self.dummy_pattern:
                # 악성코드 패턴이 같다면 결과 값을 리턴한다.
                return True, self.virus_name, 0 # 악성코드 발견 여부, 발견한 악성코드 이름, 악성코드 ID값

        except IOError:
            pass

        # 악성코드를 발견하지 못했음을 리턴한다.
        return False, '', -1 # 악성코드 발견 여부, 발견한 악성코드가 없으므로 '', 악성코드ID를 -1로 리턴
    ...
    def disinfect(self, filename, malware_id): # 악성코드 치료
        # 악성코드를 치료하는 작업이다 보니 예외적인 상황이 발생할 가능성이 크므로 try, except로 처리
        try:
            # 악성코드 진단 결과에서 받은 ID 값이 0안가?
            if malware_id == 0:
