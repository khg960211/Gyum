...
def __ext_euclid(a, b):
    ...
    while 1:
        ...
        list_q.append(list_r[i - 2] / list_r[i - 2])
...
#-------------------------------------------------------------------------------
# create_key(pu_fname='key.prk', pr_fname='key_skr', debug)
# rsa 키를 생성한다.
# 인자값 : pu_fname - 공개키 파일 이름
#           pr_fname - 개인키 파일 이름
# 리턴값 : 키 생성 성공 여부
#-------------------------------------------------------------------------------
def create_key(pu_fname='key.prk', pr_fname='key_skr', debug=False):
    p = __gen_prime(128) # 128비트 소수 생성
    q = __gen_prime(128) # 128비트 소수 생성
